...
const getSkillTemplate = (skillName, skillTitle) => `---
name: ${skillName}
...
async function initSkill(skillName, targetPath) {
    /**
    ...
    const skillDir = path.join(absolutePath, skillName);
    ...
    try {
        ...
        // If error is ENOENT, file doesn't exist, which is what we want
        if (e.code !== 'ENOENT') {
    ...
    try {
        ...
        await fs.writeFile(exampleScriptPath, getExampleScript(skillName), 'utf8');
        ...
        await fs.writeFile(exampleReferencePath, getExampleReference(skillTitle), 'utf8');
...
async function main() {
    // process.argv structure: [node_bin, script_path, arg1, arg2...]
